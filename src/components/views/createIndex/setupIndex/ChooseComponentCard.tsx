...
import { strategicSafty } from "../../../../utils/strategicSafty";
...
const ChooseComponentCard = (props: IProps) => {
  ...
  const [showChooseComponentModal, setShowChooseComponentModal] =
    ...
    function removeSmallArrayElements(big: Asset[], small: Asset[]) {
      return big.filter(function (bigElement) {
        return !small.some(function (smallElement) {
          return (
            bigElement.address === smallElement.address &&
            bigElement.address === smallElement.address
          );
        });
      });
  ...
  const handleAddToken = async (asset: Asset) => {
    ...
    // setComponentList(
    //   newComponentList.map((elem) => ({
    //     ...elem,
    //     allocation: newAllocation,
    //     locked: false,
    //   }))
    // );
    setComponentList(calculateRatios(newComponentList));
    setShowChooseComponentModal(false);
  };
  ...
  const handleOnAllocation = (idx: number, value: string) => {
    ...
    setComponentList(
      strategicSafty(
        newComponentList.map((cmp) => ({
          ...cmp,
          allocation: Number(cmp.allocation.toFixed(2)),
        }))
      )
    );
  };
  ...
  function canSlide() {
    ...
    for (let i = 0; i < length; i++) {
      if (componentList[i].locked) countLocked += 1;
    }
    if (countLocked >= length - 1) return false;
    return true;
  }
  ...
  return (
    <Box>
      ...
      <Button sx={{ width: "100%" }} onClick={handelOpenChooseCompoentModal}>
        Search for token
      </Button>
      <Modal
      ...
      <Box sx={{ padding: "20px", overflow: "auto", maxHeight: "500px" }}>
        ...
        {componentList.map((token: ComponentList, idx: number) => (
          <Card
            key={idx}
            sx={{ padding: "10px", backgroundColor: "rgba(255,255,255,0.75)" }}
          >
            <Grid container>
